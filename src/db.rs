...
pub fn update_todo(conn: &mut rusqlite::Connection, item: &Todo) -> Result<usize> {
    let (sql, values) = Query::update()
        ...
        .and_where(Expr::col("id").eq(item.id.as_str()))
        .build_rusqlite(SqliteQueryBuilder);
...
pub fn delete_todo(conn: &mut rusqlite::Connection, todo_id: &String) -> Result<usize> {
    let (sql, values) = Query::delete()
        .from_table("todos")
        .and_where(Expr::col("id").eq(todo_id.as_str()))
        .build_rusqlite(SqliteQueryBuilder);
...
pub fn toggle_todo(conn: &mut rusqlite::Connection, item_id: &String) -> Result<usize> {
    ...
    let (sql, values) = Query::update()
        ...
        .and_where(Expr::col("id").eq(item_id.as_str()))
        .build_rusqlite(SqliteQueryBuilder);
...
pub fn read_todo(conn: &mut rusqlite::Connection, todo_id: &String) -> Result<Todo> {
    let (sql, values) = Query::select()
        .columns(["id", "title", "completed"])
        .from("todos")
        .and_where(Expr::col("id").eq(todo_id.as_str()))
